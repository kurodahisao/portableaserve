...
(defun unpublish-file (path &key (server *wserver*) &aux value)
  (dolist (locator (wserver-locators server) value)
    (when (typep locator 'locator-exact)
      (let ((hash (locator-info locator)))
        (if (remhash path hash) (setq value t))))))

(eval-when (:load-toplevel :execute)
  (export '(unpublish-file)))
