...
(defvar *default-aserve-external-format* :utf-8)
  ...
  (if* ssl
     then (if* (pathnamep ssl)
	  ...
	  (setq accept-hook 
        ...
        #+sbcl
        (funcall 'cl+ssl:make-ssl-server-stream socket
                 :certificate ssl
...
(eval-when (:load-toplevel :execute)
  (setq *log-time-zone* local-time:*default-timezone*))     

(defun maybe-universal-time-to-date (ut-or-string &optional (time-zone *log-time-zone*))
...
(defun universal-time-to-date (ut &optional (time-zone *log-time-zone*))
    ...
    (if* (and (eql ut (caar cval))
       ...
       then ; turns out we often repeatedly ask for the same conversion
		...
		(let ((ans
		       (format
			nil
			#+ignore "~a, ~2,'0d ~a ~d ~2,'0d:~2,'0d:~2,'0d~@[ GMT~]"
                        "~a, ~2,'0d ~a ~d ~2,'0d:~2,'0d:~2,'0d~[~;~; AEST~; JST~; CST~;~;~;~;~;~;~;~; GMT~]"
